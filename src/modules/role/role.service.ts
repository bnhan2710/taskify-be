import {
  ...
  NotFoundError,
} from '../../core/handler/error.response';
...
class RoleService {
  public async CreateRole(createRoleDto: IRoleDto): Promise<void> {
    const existsRole = await connection
      .getRepository(Role)
      .findOne({ where: { name: createRoleDto.roleName } });
    if (existsRole) {
  ...
  public async CreatePermission(createPermissionDto: IPermissionDto) {
    const existsPermission = await connection
      .getRepository(Permission)
      .findOne({ where: { name: createPermissionDto.permissionName } });
    if (existsPermission) {
  ...
  public async insertUserBoardRole(userId: string, boardId: string) {
    const userRole = await connection
      ...
      .findOne({ where: { userId: userId, boardId: boardId } });
    if (userRole) {
    ...
    await connection
      .getRepository(BoardUserRole)
      .save({ userId: userId, boardId: boardId, roleId: onwer.id });
  }
